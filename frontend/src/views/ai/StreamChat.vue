  ...
  <div class="page-container ai-chat-page">
    <div class="page-card mode-banner">
      <div class="mode-banner-left">
        ...
        <div>
          <h3 class="mode-banner-title">流式对话</h3>
          <p class="mode-banner-desc">SSE 流式输出，融合限流 + 并发控制 + 缓存，逐字实时返回</p>
        </div>
      </div>
      <el-tag type="info" size="small" effect="plain">SSE 流式</el-tag>
    </div>
...
const sendMessage = async () => {
  ...
  try {
    ...
    while (true) {
      ...
      if (done) {
        console.log('[SSE] 流结束, 共接收', chunkCount, '个chunk')
        break
      }

      chunkCount++
      const chunkText = decoder.decode(value, { stream: true })
      console.log('[SSE] 收到chunk #' + chunkCount, '长度:', chunkText.length, '内容:', chunkText.substring(0, 100))
...
onMounted(() => {
  messages.value = [{
    role: 'assistant',
    content: '你好！当前使用的是「流式对话」模式。SSE 流式输出，融合限流 + 并发控制 + 缓存，逐字实时返回，请输入你的问题。'
  }]
})
